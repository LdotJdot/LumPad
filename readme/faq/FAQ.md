...
There are a few easy-to-miss touchpoints: add comment-toggle arms and lexer property setters in `src/EditLexer.c`, register the lexer in `Styles.c`, and add a `[<pszName>]` theme block for every theme INI (the lexer’s 4th `EDITLEXER` field, not the display name string).

---

## License

Notepad3 is free software released under the **BSD 3-Clause License** — see [`Build/Docs/License.txt`](../../Build/Docs/License.txt).
