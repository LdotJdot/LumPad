@ECHO OFF
SETLOCAL ENABLEEXTENSIONS
:: encoding: UTF-8
CHCP 65001 >NUL 2>&1

rem ******************************************************************************
rem *                                                                            *
rem * Notepad3                                                                   *
rem *                                                                            *
rem * make_portable_alpha(.7z).cmd                                               *
rem *   Batch file for creating "Portable alpha (*.7z)" packages                 *
rem *                                                                            *
rem * See License.txt for details about distribution and modification.           *
rem *                                                                            *
rem *                                                 (c) Rizonesoft 2008-2026   *
rem *                                                   https://rizonesoft.com   *
rem *                                                                            *
rem ******************************************************************************

PUSHD %~dp0

rem SET SEVENZIP=%~dp07za.exe

rem Check for the help switches
IF /I "%~1" == "help"   GOTO SHOWHELP
IF /I "%~1" == "help"   GOTO SHOWHELP
IF /I "%~1" == "/help"  GOTO SHOWHELP
IF /I "%~1" == "-help"  GOTO SHOWHELP
IF /I "%~1" == "--help" GOTO SHOWHELP
IF /I "%~1" == "/?"     GOTO SHOWHELP

SET INPUTDIRx86="bin\Release_x86_v145"
SET INPUTDIRx64="bin\Release_x64_v145"
SET TEMP_NAME="make_portable_temp"

IF NOT EXIST "..\%INPUTDIRx86%\LumPad.exe" IF NOT EXIST "..\%INPUTDIRx86%\Notepad3.exe" CALL :SUBMSG "ERROR" "Compile main app x86 first!"
IF NOT EXIST "..\%INPUTDIRx64%\LumPad.exe" IF NOT EXIST "..\%INPUTDIRx64%\Notepad3.exe" CALL :SUBMSG "ERROR" "Compile main app x64 first!"

CALL :SubGetVersion
CALL :SubDetectSevenzipPath

IF /I "%SEVENZIP%" == "" CALL :SUBMSG "ERROR" "7za wasn't found!"

CALL :SubZipFiles %INPUTDIRx86% x86
CALL :SubZipFiles %INPUTDIRx64% x64

rem Compress everything into a single ZIP file
PUSHD "packages"
IF EXIST "Notepad3_%NP3_VER%.7z" DEL "Notepad3_%NP3_VER%.7z"
IF EXIST "%TEMP_NAME%"      RD /S /Q "%TEMP_NAME%"
IF NOT EXIST "%TEMP_NAME%"  MD "%TEMP_NAME%"

IF EXIST "Notepad3_%NP3_VER%*.7z" COPY /Y /V "Notepad3_%NP3_VER%*.7z" "%TEMP_NAME%\" >NUL
IF EXIST "%TEMP_NAME%\Notepad3_%NP3_VER%*.7z" DEL /F /Q "Notepad3_%NP3_VER%*.7z" >NUL

PUSHD "%TEMP_NAME%"

"%SEVENZIP%" a -t7z -ms=on -mx=7 Notepad3_%NP3_VER%.7z * >NUL
IF %ERRORLEVEL% NEQ 0 CALL :SUBMSG "ERROR" "Compilation failed!"

CALL :SUBMSG "INFO" "Notepad3_%NP3_VER%_Portable.7z created successfully!"

MOVE /Y "Notepad3_%NP3_VER%.7z" "..\Notepad3_%NP3_VER%_alpha_Portable.7z" >NUL

POPD
IF EXIST "%TEMP_NAME%" RD /S /Q "%TEMP_NAME%"

POPD

:END
TITLE Finished!
ECHO.

:: Pause of 8 seconds to verify the logfile before exiting 
:: ===========================================================================================
ping -n 9 127.0.0.1>nul

POPD
ENDLOCAL
::PAUSE
EXIT /B


:SubZipFiles
SET "ZIP_NAME=LumPad_%NP3_VER%_alpha_%2%_Portable"
TITLE Creating %ZIP_NAME%.7z...
CALL :SUBMSG "INFO" "Creating %ZIP_NAME%.7z..."
IF EXIST "%TEMP_NAME%"     RD /S /Q "%TEMP_NAME%"
IF NOT EXIST "%TEMP_NAME%" MD "%TEMP_NAME%"
IF NOT EXIST "Packages"    MD "Packages"

COPY /Y /V "LumPad_ptb.ini" "%TEMP_NAME%\"
IF EXIST "..\%1\LumPad.exe" (COPY /Y /V "..\%1\LumPad.exe" "%TEMP_NAME%\LumPad.exe") ELSE (COPY /Y /V "..\%1\Notepad3.exe" "%TEMP_NAME%\LumPad.exe")
IF EXIST "%TEMP_NAME%\LumPad_ptb.ini" RENAME "%TEMP_NAME%\LumPad_ptb.ini" "LumPad.ini"

SET "LNG=%TEMP_NAME%\lng"
SET "THEMES=%TEMP_NAME%\Themes"
SET "DOCS=%TEMP_NAME%\Docs"
IF NOT EXIST %LNG% MD %LNG%
IF NOT EXIST %THEMES% MD %THEMES%
IF NOT EXIST %DOCS% MD %DOCS%
XCOPY /E /Y /V "..\%1\lng" "%LNG%" /EXCLUDE:Ignore.txt
XCOPY /E /Y /V "Themes" "%THEMES%"
XCOPY /E /Y /V "Docs" "%DOCS%"
COPY /Y /V "Changes.txt" "%DOCS%"

SET "FAVORITES=%TEMP_NAME%\Favorites"
IF NOT EXIST "%FAVORITES%" MD "%FAVORITES%"

PUSHD "%TEMP_NAME%"
"%SEVENZIP%" a -t7z -m0=lzma2 -ms=on -mx=9^
 "%ZIP_NAME%.7z" "LumPad.exe" "LumPad.ini"^
 "Favorites" "lng" "Themes" "Docs">NUL
IF %ERRORLEVEL% NEQ 0 CALL :SUBMSG "ERROR" "Compilation failed!"

CALL :SUBMSG "INFO" "%ZIP_NAME%.7z created successfully!"

MOVE /Y "%ZIP_NAME%.7z" "..\packages" >NUL
POPD
IF EXIST "%TEMP_NAME%" RD /S /Q "%TEMP_NAME%"
EXIT /B


:SubDetectSevenzipPath
FOR %%G IN (7z.exe) DO (SET "SEVENZIP_PATH=%%~$PATH:G")
IF EXIST "%SEVENZIP_PATH%" (SET "SEVENZIP=%SEVENZIP_PATH%" & EXIT /B)

FOR %%G IN (7za.exe) DO (SET "SEVENZIP_PATH=%%~$PATH:G")
IF EXIST "%SEVENZIP_PATH%" (SET "SEVENZIP=%SEVENZIP_PATH%" & EXIT /B)

FOR /F "tokens=2*" %%A IN (
  'REG QUERY "HKLM\SOFTWARE\7-Zip" /v "Path" 2^>NUL ^| FIND "REG_SZ" ^|^|
   REG QUERY "HKLM\SOFTWARE\Wow6432Node\7-Zip" /v "Path" 2^>NUL ^| FIND "REG_SZ"') DO SET "SEVENZIP=%%B7z.exe"
EXIT /B


:SubGetVersion
rem Get the version
FOR /F "tokens=3,4 delims= " %%K IN (
  'FINDSTR /I /L /C:"define VERSION_MAJOR" "..\src\VersionEx.h"') DO (SET "VerMajor=%%K")
FOR /F "tokens=3,4 delims= " %%K IN (
  'FINDSTR /I /L /C:"define VERSION_MINOR" "..\src\VersionEx.h"') DO (SET "VerMinor=%%K")
FOR /F "tokens=3,4 delims= " %%K IN (
  'FINDSTR /I /L /C:"define VERSION_REV" "..\src\VersionEx.h"') DO (SET "VerRev=%%K")
FOR /F "tokens=3,4 delims= " %%K IN (
  'FINDSTR /I /L /C:"define VERSION_BUILD" "..\src\VersionEx.h"') DO (SET "VerBuild=%%K")

SET NP3_VER=%VerMajor%.%VerMinor%.%VerRev%.%VerBuild%
EXIT /B


:SHOWHELP
TITLE %~nx0 %1
ECHO. & ECHO.
ECHO Usage:  %~nx0 [VS2010^|VS2012^|VS2013^|VS2015^|WDK]
ECHO.
ECHO Notes:  You can also prefix the commands with "-", "--" or "/".
ECHO         The arguments are not case sensitive.
ECHO. & ECHO.
ECHO Executing %~nx0 without any arguments is equivalent to "%~nx0 WDK"
ECHO.
ENDLOCAL
EXIT /B


:SUBMSG
ECHO. & ECHO ______________________________
ECHO [%~1] %~2
ECHO ______________________________ & ECHO.
IF /I "%~1" == "ERROR" (
  PAUSE
  EXIT
) ELSE (
  EXIT /B
)
