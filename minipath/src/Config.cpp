        ...
        // Application Data (%APPDATA%)
        {
            ...
            if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0, nullptr, &pszPath))) {
                ...
                if (SUCCEEDED(PathCchAppend(tchBuild, COUNTOF(tchBuild), L"Rizonesoft")) &&
                    SUCCEEDED(PathCchAppend(tchBuild, COUNTOF(tchBuild), L"LumPad")) &&
...
int FindIniFile()
{
    ...
    // --- check for LumPad.ini to synchronize some settings ---
    if (SUCCEEDED(PathCchRemoveFileSpec(tchModule, COUNTOF(tchModule)))) {
        StringCchCat(tchModule, COUNTOF(tchModule), L"\\LumPad.exe");
    }
    StringCchCopy(tchTest, COUNTOF(tchTest), PathFindFileName(tchModule));
    if (FAILED(PathCchRenameExtension(tchTest, COUNTOF(tchTest), L".ini"))) {
        StringCchCopy(tchTest, COUNTOF(tchTest), L"LumPad.ini");
    }
    bFound = CheckIniFile(tchTest, tchModule);
    if (!bFound) {
        StringCchCopy(tchTest, COUNTOF(tchTest), L"LumPad.ini");
    ...
    if (bFound) {
        // allow two redirections: administrator -> user -> custom
        if (CheckIniFileRedirect(L"LumPad", L"LumPad.ini", tchTest, tchModule)) {
            CheckIniFileRedirect(L"LumPad", L"LumPad.ini", tchTest, tchModule);
        }
...
int TestIniFile()
{
    ...
    // --- test for LumPad.ini (legacy Notepad3.ini) ---
    if (PathIsDirectory(g_wchNP3IniFile) || *CharPrev(g_wchNP3IniFile, StrEnd(g_wchNP3IniFile)) == L'\\') {
        ...
        if (SUCCEEDED(PathCchRemoveFileSpec(wchModule, COUNTOF(wchModule)))) {
            StringCchCat(wchModule, COUNTOF(wchModule), L"\\LumPad.exe");
        }
        ...
        if (!PathIsExistingFile(g_wchNP3IniFile)) {
            StringCchCopy(PathFindFileName(g_wchNP3IniFile), COUNTOF(g_wchNP3IniFile) - (PathFindFileName(g_wchNP3IniFile) - g_wchNP3IniFile), L"LumPad.ini");
            if (!PathIsExistingFile(g_wchNP3IniFile)) {
...
void LoadFlags()
{
    __try {
        ...
        if (!IniSectionGetString(Settings_Section2, PrefLngLocName, L"", g_UsedLngLocaleName, LOCALE_NAME_MAX_LENGTH)) {
            // try to fetch Locale Name from LumPad.ini
...
void LoadSettings()
{
    __try {
        ...
        if (!IniSectionGetString(Settings_Section, L"Favorites", Defaults.tchFavoritesDir,
            ...
            // try to fetch Favorites dir from LumPad.ini
        ...
        if (IniSectionGetString(Settings_Section, L"OpenWithDir", L"",
            ...
            // try to fetch Open With dir from LumPad.ini
