        ...
        for (int iLexer = 0; iLexer < COUNTOF(g_pLexArray); iLexer++) {
            ...
            if (cchRead >= (size_t)STYLE_EXTENSIONS_BUFFER) {
                ...
                StringCchPrintf(warnMsg, COUNTOF(warnMsg),
                    L"LumPad: FileNameExtensions for [%s] is %zu chars, exceeds %d-char limit; truncating.\n",
            ...
            if (_MigrateLegacyRegexEntries(g_pLexArray[iLexer]->szExtensions)) {
                ...
                StringCchPrintf(migMsg, COUNTOF(migMsg),
                    L"LumPad: migrated legacy '\\regex' entries in FileNameExtensions for [%s]; will be rewritten on next save.\n",
                    Lexer_Section);
