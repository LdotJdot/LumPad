...
// IniFileSet* / IniFileDelete write the on-disk INI via a separate CSimpleIni instance.
// If s_INI is already loaded (between OpenSettingsFile / CloseSettingsFile or from startup),
// that cache would otherwise stay stale; a later SaveIniFileCache() would overwrite the file
// and drop keys that only existed on disk (e.g. [Suppressed Messages], [Session]).
static bool _IniPathTargetsActiveIniCache(const HPATHL hpthIniFile)
{
    if (!IsIniFileCached() || Path_IsEmpty(hpthIniFile) || Path_IsEmpty(Paths.IniFile)) {
        return false;
    }
    if (Path_StrgComparePath(hpthIniFile, Paths.IniFile, Paths.WorkingDirectory, true) == 0) {
        return true;
    }
    if (Path_StrgComparePath(hpthIniFile, Paths.IniFile, Paths.ModuleDirectory, true) == 0) {
        return true;
    }
    // Last resort: normalized string compare only (portable dir vs cwd mismatches).
    return Path_StrgComparePath(hpthIniFile, Paths.IniFile, Paths.WorkingDirectory, false) == 0;
}
...
extern "C" bool IniFileSetString(const HPATHL hpthIniFile, LPCWSTR lpSectionName, LPCWSTR lpKeyName, LPCWSTR lpString)
{
    ...
    bool const ok = SI_Success(rc);
    if (ok && _IniPathTargetsActiveIniCache(hpthIniFile)) {
        (void)s_INI.SetValue(lpSectionName, lpKeyName, lpString, nullptr, !s_bUseMultiKey);
    }
    return ok;
}
// ============================================================================
...
extern "C" bool IniFileSetLong(const HPATHL hpthIniFile, LPCWSTR lpSectionName, LPCWSTR lpKeyName, long lValue)
{
    ...
    bool const ok = SI_Success(rc);
    if (ok && _IniPathTargetsActiveIniCache(hpthIniFile)) {
        (void)s_INI.SetLongValue(lpSectionName, lpKeyName, lValue, nullptr, false, !s_bUseMultiKey);
    }
    return ok;
}
// ============================================================================
...
extern "C" bool IniFileSetBool(const HPATHL hpthIniFile, LPCWSTR lpSectionName, LPCWSTR lpKeyName, bool bValue)
{
    ...
    bool const ok = SI_Success(rc);
    if (ok && _IniPathTargetsActiveIniCache(hpthIniFile)) {
        (void)s_INI.SetBoolValue(lpSectionName, lpKeyName, bValue, nullptr, !s_bUseMultiKey);
    }
    return ok;
}
// ============================================================================
...
extern "C" bool IniFileDelete(const HPATHL hpthIniFile, LPCWSTR lpSectionName, LPCWSTR lpKeyName, bool bRemoveEmpty)
{
    ...
    bool const ok = SI_Success(rc);
    if (ok && _IniPathTargetsActiveIniCache(hpthIniFile)) {
        (void)s_INI.Delete(lpSectionName, lpKeyName, bRemoveEmpty);
    }
    return ok;
}
// ============================================================================
...
static bool _CheckAndSetIniFile(HPATHL hpth_in_out)
{
    ...
    if (!result) {
        ...
        if (Path_GetKnownFolder(FOLDERID_RoamingAppData, happdata)) {
            ...
            Path_Append(happdata, _W(SAPPNAME));
...
extern "C" bool FindIniFile()
{
    ...
    SetEnvironmentVariableW(LUMPAD_MODULE_DIR_ENV_VAR, Path_Get(hdir_pth));
...
extern "C" bool TestIniFile()
{
    ...
    if (!Path_IsExistingFile(Paths.IniFile)) {
        // Remember the intended portable path for "Save Settings Now" / redirect edge cases.
        Path_Reset(Paths.IniFileDefault, Path_Get(Paths.IniFile));
        // Do NOT clear Paths.IniFile here: wWinMain calls CreateIniFile(Paths.IniFile) immediately
        // after TestIniFile(). Clearing left no writable INI path (Globals.bCanSaveIniFile stayed
        // false), so suppressed dialogs, themes, WarnInconsistEOLs, etc. could never persist.
        return false;
    }
