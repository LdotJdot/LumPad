...
static void LumPad_ProcessDeferredMultiOpenTabs(HWND hwnd);
static void LumPad_RequestFlushDeferredCopyData(HWND hwnd);
static void LumPad_OnFlushDeferredCopyData(HWND hwnd);
static void LumPad_CancelDeferredCopyDataQueue(void);
static void LumPad_SetStartupBulkRedraw(HWND hwndMain, BOOL fRedraw);
static bool LumPad_RestoreSessionTabs(HWND hwnd);
static void LumPad_SaveSessionTabs(void);
static bool LumPad_TryStartupForwardToRunningInstance(void);
static LRESULT CALLBACK Lumpad_LLKbdProc(int nCode, WPARAM wParam, LPARAM lParam);
static void         Lumpad_InstallDoubleCtrlHook(HWND hwndTarget);
...
#define TBBUTTON_LAST_DEFAULT (36)

WCHAR              TBBUTTON_DEFAULT_IDS[256] = { L'\0' };  // filled in _InitGlobals()
const WCHAR* const TBBUTTON_DEFAULT_IDS_OLD = L"1 32 2 4 28 0 5 6 0 7 8 9 0 10 11 0 30 0 12 0 24 26 0 22 23 0 13 14 0 27 0 15 0 25 0 17";
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nShowCmd)
{
    ...
    // Forward startup files to an already-running window (e.g. drag-drop onto .exe)
    if (LumPad_TryStartupForwardToRunningInstance()) {
    ...
    Lumpad_InstallDoubleCtrlHook(hwnd);
...
static BOOL CALLBACK _EnumWndProc(HWND hwnd, LPARAM lParam)
{
    ...
    if (GetClassName(hwnd, szClassName, COUNTOF(szClassName))) {
        ...
        if (StrCmpW(szClassName, s_wchWndClass) == 0) {
            // Do not gate on IDC_REUSELOCK / REUSEWINDOWLOCKTIMEOUT: FileLoad() calls
            // _SetEnumWindowsItems() after every successful open, which refreshed the lock
            // to "now" and hid this HWND from peers for ~1s — rapid Explorer launches then
            // spawned extra processes instead of WM_COPYDATA tab routing.
            // Do not skip "hung" windows: during heavy loads Windows may mark us hung while
            // copies still route via WM_COPYDATA (now non-blocking); skipping hid the only
            // instance and spawned duplicate LumPad.exe processes.
            *(HWND*)lParam = hwnd;
...
HWND InitInstance(const HINSTANCE hInstance, int nCmdShow)
{
    ...
    if (!bOpened && Path_IsEmpty(s_pthArgFilePath) && !Settings.AutoLoadMRUFile && !s_flagNewFromClipboard && !s_flagPasteBoard) {
        bOpened = LumPad_RestoreSessionTabs(hwndMain);
    }
    ...
    if (s_cFileList > 1) {
        LumPad_ProcessDeferredMultiOpenTabs(hwndMain);
    }
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
...
#ifdef D_NP3_WIN10_DARK_MODE
    case WM_ERASEBKGND:
        // Fills gaps between children (e.g. 1px seam under the tab strip) that would otherwise show COLOR_WINDOW.
        if (IsDarkModeSupported() && UseDarkMode()) {
            RECT rc = { 0 };
            GetClientRect(hwnd, &rc);
            HDC const hdc = (HDC)wParam;
            HBRUSH const br = CreateSolidBrush(GetModeBtnfaceColor(true));
            if (br) {
                FillRect(hdc, &rc, br);
                DeleteObject(br);
            }
    ...
    case WM_LUMPAD_DOUBLE_CTRL_FOCUS:
        if (IsIconic(hwnd)) {
    ...
    case WM_LUMPAD_FLUSH_DEFERRED_COPYDATA:
        LumPad_OnFlushDeferredCopyData(hwnd);
        break;
    ...
    case WM_CLOSE:
        ...
        if (TabDoc_PreCloseSaveAll(hwnd, FSF_None)) {
            LumPad_CancelDeferredCopyDataQueue();
    ...
    case WM_QUERYENDSESSION:
        ...
        else if (TabDoc_PreCloseSaveAll(hwnd, FSF_EndSession)) {
            return TRUE;
        }
        break;
...
//=============================================================================
//
//  LumPad_SaveSessionTabs()
//
//  Minimal session restore: persist open file paths (not untitled buffers).
//
static void LumPad_SaveSessionTabs(void)
{
    if (!Globals.bCanSaveIniFile || Path_IsEmpty(Paths.IniFile)) {
        return;
    }
    int const tabCount = TabDoc_GetTabCount();
    int fileCount = 0;
    for (int i = 0; i < tabCount; ++i) {
        if (StrIsNotEmpty(TabDoc_GetTabPath(i))) {
            ++fileCount;
        }
    }
    IniFileSetLong(Paths.IniFile, L"Session", L"Count", fileCount);
    int outIdx = 0;
    for (int i = 0; i < tabCount; ++i) {
        LPCWSTR const p = TabDoc_GetTabPath(i);
        if (StrIsNotEmpty(p)) {
            WCHAR key[32] = { L'\0' };
            StringCchPrintfW(key, COUNTOF(key), L"File%i", outIdx);
            IniFileSetString(Paths.IniFile, L"Session", key, p);
            ++outIdx;
        }
    }
    IniFileSetString(Paths.IniFile, L"Session", L"Active", Path_Get(Paths.CurrentFile));
}

//=============================================================================
//
//  LumPad_SetStartupBulkRedraw()
//
//  Suppress painting while loading many tabs (session restore / multi-file open)
//  so controls do not flash tab-by-tab; pair with TRUE then RedrawWindow.
//
static void LumPad_SetStartupBulkRedraw(HWND hwndMain, BOOL fRedraw)
{
    if (hwndMain && IsWindow(hwndMain)) {
        SendMessageW(hwndMain, WM_SETREDRAW, fRedraw, 0);
    }
    if (s_hwndEditFrame && IsWindow(s_hwndEditFrame)) {
        SendMessageW(s_hwndEditFrame, WM_SETREDRAW, fRedraw, 0);
    }
    if (Globals.hwndEdit && IsWindow(Globals.hwndEdit)) {
        SendMessageW(Globals.hwndEdit, WM_SETREDRAW, fRedraw, 0);
    }
    TabDoc_SetStripRedraw(fRedraw);
}

//=============================================================================
//
//  LumPad_RestoreSessionTabs()
//
static bool LumPad_RestoreSessionTabs(HWND hwnd)
{
    ...
    long const count = IniFileGetLong(Paths.IniFile, L"Session", L"Count", 0);
    if (count <= 0) {
        return false;
    }
    WCHAR wchActive[MAX_PATH_EXPLICIT] = { L'\0' };
    IniFileGetString(Paths.IniFile, L"Session", L"Active", L"", wchActive, COUNTOF(wchActive));

    LumPad_SetStartupBulkRedraw(hwnd, FALSE);

    bool openedAny = false;
    for (long i = 0; i < count; ++i) {
        WCHAR key[32] = { L'\0' };
        StringCchPrintfW(key, COUNTOF(key), L"File%li", i);
        WCHAR wchFile[PATHLONG_MAX_CCH] = { L'\0' };
        if (IniFileGetString(Paths.IniFile, L"Session", key, L"", wchFile, COUNTOF(wchFile)) == 0) {
            continue;
        }
        HPATHL hp = Path_Allocate(wchFile);
        if (!hp) {
            continue;
        }
        Path_NormalizeEx(hp, Paths.WorkingDirectory, true, Flags.bSearchPathIfRelative);
        FileLoadFlags flg = FLF_None;
        ...
        if (!openedAny) {
            openedAny = FileLoad(hp, flg, 0, 0);
        } else {
            if (TabDoc_NewTabForNextDocument(hwnd)) {
                (void)FileLoad(hp, flg, 0, 0);
            }
        }
        Path_Release(hp);
    }
    if (openedAny && StrIsNotEmpty(wchActive)) {
        HPATHL hpAct = Path_Allocate(wchActive);
        if (hpAct) {
            Path_NormalizeEx(hpAct, Paths.WorkingDirectory, true, Flags.bSearchPathIfRelative);
            int const idx = TabDoc_FindTabByPath(hpAct);
            if (idx >= 0) {
                TabDoc_SwitchToTab(hwnd, idx);
            }
            Path_Release(hpAct);
        }
    }

    LumPad_SetStartupBulkRedraw(hwnd, TRUE);
...
#ifdef D_NP3_WIN10_DARK_MODE
    bool const dmFrame = (IsDarkModeSupported() && UseDarkMode());
#else
    bool const dmFrame = false;
#endif
    // Explorer-themed ListView draws a light border under dark UI; drop theme on the frame in dark mode.
    InitWindowCommon(s_hwndEditFrame, !dmFrame);
#ifdef D_NP3_WIN10_DARK_MODE
    if (dmFrame) {
        SetWindowTheme(s_hwndEditFrame, L"", L"");
        if (IsDarkModeSupported()) {
            AllowDarkModeForWindowEx(s_hwndEditFrame, true);
        }
    }
#endif
...
#ifdef D_NP3_WIN10_DARK_MODE
    if (s_hwndEditFrame && IsWindow(s_hwndEditFrame)) {
        LONG_PTR const exf = GetWindowLongPtr(s_hwndEditFrame, GWL_EXSTYLE);
        if (dmFrame) {
            SetWindowLongPtr(s_hwndEditFrame, GWL_EXSTYLE, exf & ~(LONG_PTR)WS_EX_CLIENTEDGE);
        } else {
            SetWindowLongPtr(s_hwndEditFrame, GWL_EXSTYLE, exf | (LONG_PTR)WS_EX_CLIENTEDGE);
        }
        SetWindowPos(s_hwndEditFrame, NULL, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOMOVE | SWP_NOSIZE | SWP_FRAMECHANGED);
        InvalidateRect(s_hwndEditFrame, NULL, TRUE);
    }
#endif
}
...
{
    if (nCode != HC_ACTION) {
        return CallNextHookEx(s_hhkDoubleCtrlLL, nCode, wParam, lParam);
    }
    KBDLLHOOKSTRUCT* const p = (KBDLLHOOKSTRUCT*)lParam;
    if (s_hwndDoubleCtrlLLTarget && IsWindow(s_hwndDoubleCtrlLLTarget)) {
...
static void _SetEnumWindowsItems(HWND hwnd)
{
    ...
    // Intentionally do not touch IDC_REUSELOCK here — see _EnumWndProc.
}
...
LRESULT MsgSize(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    int const cyTab = TabDoc_GetStripHeight();
    int padEditTop = 0;
    if (cyTab > 0) {
        TabDoc_LayoutStrip(x, y, cx);
        y += cyTab;
        cy -= cyTab;
        // Breathing room between tab strip and first text line (avoids a few px of visual clipping).
        padEditTop = max_i(1, ScaleIntToDPI(hwnd, 2));
    }
    ...
    DeferWindowPos(hdwp, g_hwndEditWindow, s_hwndEditFrame,
                   x + s_cxEditFrame, y + s_cyEditFrame + padEditTop, cx - 2 * s_cxEditFrame, cy - 2 * s_cyEditFrame - padEditTop,
...
LRESULT MsgDropFiles(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if (hDrop) {
        ...
        for (UINT i = 0; i < cnt; ++i) {
            ...
            if (vkCtrlDown) {
                WININFO wi = GetMyWindowPlacement(hwnd, NULL, (offset * (i + 1)), bFullVisible);
                _OnDropOneFile(hwnd, hdrop_pth, &wi);
            } else if (i == 0) {
                _OnDropOneFile(hwnd, hdrop_pth, NULL);
            } else if (Path_IsExistingFile(hdrop_pth)) {
                HPATHL hp = Path_Copy(hdrop_pth);
                if (hp && TabDoc_NewTabForNextDocument(hwnd)) {
                    FileLoadFlags flg = FLF_None;
                    flg |= Settings.SkipUnicodeDetection ? FLF_SkipUnicodeDetect : 0;
                    flg |= Settings.SkipANSICodePageDetection ? FLF_SkipANSICPDetection : 0;
                    (void)FileLoad(hp, flg, 0, 0);
...
// WM_COPYDATA cross-process delivery uses SendMessage; running FileLoad() (and modal dialogs)
// inside the handler blocks the sender and can mark this process as "not responding", so other
// LumPad.exe instances skip us in EnumWindows (IsHungAppWindow). Architecture: copy the full
// np3params blob, enqueue, return immediately; apply opens from WM_LUMPAD_FLUSH_DEFERRED_COPYDATA
// one file per posted message (keeps the UI responsive).
#define LUMPAD_COPYDATA_DEFER_MAX 48

typedef struct LumPadDeferredNp3Blob {
    BYTE* data;
    DWORD cb;
} LumPadDeferredNp3Blob;

static LumPadDeferredNp3Blob s_deferredNp3Blob[LUMPAD_COPYDATA_DEFER_MAX];
static int                   s_cDeferredNp3Blob = 0;
static bool                  s_flushDeferredCopyDataPosted = false;

static bool LumPad_EnqueueDeferredNp3Blob(BYTE* blob, DWORD cb)
{
    if (!blob || cb < sizeof(np3params)) {
        FreeMem(blob);
        return false;
    }
    if (s_cDeferredNp3Blob >= LUMPAD_COPYDATA_DEFER_MAX) {
        FreeMem(blob);
        return false;
    }
    s_deferredNp3Blob[s_cDeferredNp3Blob].data = blob;
    s_deferredNp3Blob[s_cDeferredNp3Blob].cb = cb;
    ++s_cDeferredNp3Blob;
    return true;
}
...
static void LumPad_RequestFlushDeferredCopyData(HWND hwnd)
{
    if (!hwnd || !IsWindow(hwnd) || (s_cDeferredNp3Blob <= 0)) {
        return;
    }
    if (s_flushDeferredCopyDataPosted) {
        return;
    }
    if (PostMessageW(hwnd, WM_LUMPAD_FLUSH_DEFERRED_COPYDATA, 0, 0)) {
        s_flushDeferredCopyDataPosted = true;
    }
}

static void LumPad_ApplyNp3ParamsBlob(HWND hwnd, LPnp3params params)
{
    ...
    if (bOpened) {
        ...
        if (params->flagTitleExcerpt) {
            StringCchCopyN(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt), StrEnd(Path_Get(hfile_pth), 0) + 1,
                COUNTOF(s_wchTitleExcerpt));
        }
    }
    Encoding_Forced(CPI_NONE);
...
static void LumPad_OnFlushDeferredCopyData(HWND hwnd)
{
    s_flushDeferredCopyDataPosted = false;
    if (s_cDeferredNp3Blob <= 0) {
        return;
    }

    BYTE* blob = s_deferredNp3Blob[0].data;
    if (s_cDeferredNp3Blob > 1) {
        memmove(&s_deferredNp3Blob[0], &s_deferredNp3Blob[1],
            (size_t)(s_cDeferredNp3Blob - 1) * sizeof(s_deferredNp3Blob[0]));
    }
    --s_cDeferredNp3Blob;
    s_deferredNp3Blob[s_cDeferredNp3Blob].data = NULL;
    s_deferredNp3Blob[s_cDeferredNp3Blob].cb = 0;

    LPnp3params params = (LPnp3params)blob;
    LumPad_ApplyNp3ParamsBlob(hwnd, params);
    FreeMem(blob);

    UpdateToolbar_Now(hwnd);
    UpdateMargins(true);

    LumPad_RequestFlushDeferredCopyData(hwnd);
}

static void LumPad_CancelDeferredCopyDataQueue(void)
{
    for (int i = 0; i < s_cDeferredNp3Blob; ++i) {
        FreeMem(s_deferredNp3Blob[i].data);
        s_deferredNp3Blob[i].data = NULL;
        s_deferredNp3Blob[i].cb = 0;
    }
    s_cDeferredNp3Blob = 0;
    s_flushDeferredCopyDataPosted = false;
}
...
LRESULT MsgCopyData(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    // Do not refresh IDC_REUSELOCK here (see _EnumWndProc / _SetEnumWindowsItems).

    if (pcds->dwData == DATA_NOTEPAD3_PARAMS) {
        if (!pcds->lpData || (pcds->cbData < sizeof(np3params))) {
            return FALSE;
        }

        LPnp3params const pPeek = (LPnp3params)pcds->lpData;
        if (pPeek->flagFileSpecified) {
            BYTE* blob = AllocMem(pcds->cbData, HEAP_ZERO_MEMORY);
            if (!blob) {
                return FALSE;
            }
            CopyMemory(blob, pcds->lpData, pcds->cbData);
            if (!LumPad_EnqueueDeferredNp3Blob(blob, pcds->cbData)) {
                return FALSE;
            }
            LumPad_RequestFlushDeferredCopyData(hwnd);
            return FALSE;
...
LRESULT MsgInitMenu(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    EnableCmd(hmenu, IDM_FILE_CLOSETAB, TabDoc_GetTabCount() >= 1);
    ...
    EnableCmd(hmenu, IDM_FILE_RECENT, (MRU_Count(Globals.pFileMRU) > 0));
    {
        ...
        if (Path_IsNotEmpty(Paths.IniFile)) {
            sessionCount = IniFileGetLong(Paths.IniFile, L"Session", L"Count", 0);
        }
        EnableCmd(hmenu, IDM_FILE_RESTORE_SESSION, (sessionCount > 0));
    }
...
static bool _HandleFileCommands(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_FILE_RESTORE_SESSION:
        if (!LumPad_RestoreSessionTabs(hwnd)) {
            InfoBoxLng(MB_OK | MB_ICONINFORMATION, NULL, IDS_MUI_INF_SESSION_RESTORE_FAIL);
        }
        break;
...
LRESULT MsgNotify(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    // Tab strip must be handled *before* SET_FCT_GUARD: that macro uses a static re-entrancy flag
    // cleared only by RESET_FCT_GUARD at the end; returning from inside the guarded block without
    // RESET would leave _fctguard true and silence all subsequent WM_NOTIFY (including Scintilla).
    const LPNMHDR pnmhEarly = (LPNMHDR)lParam;
    if (pnmhEarly && pnmhEarly->hwndFrom && IsWindow(pnmhEarly->hwndFrom)) {
        LRESULT const tabNotify = TabDoc_OnNotify(hwnd, pnmhEarly);
        if (tabNotify) {
            return tabNotify;
        }
    }
...
static void _MRU_UpdateSession(void);

// Untitled buffer has no non-whitespace characters (matches FileSave empty-doc heuristic).
static bool _IsUntitledDocEffectivelyEmpty(void)
{
...
{
    return _IsDisposableEmptyUntitledBuffer();
}

//=============================================================================
//
//  FileLoad() — skip the initial save dialog when replacing a buffer that
//  FileSave would treat as an empty/whitespace-only untitled document (same
//  heuristic as FileSave’s bIsEmptyNewFile + bSaveNeeded).
//
static bool _FileLoad_ShouldSkipInitialSavePrompt(bool const bReloadFile)
{
    // Untitled buffers: never let IsFileChangedFlagSet() block the skip path (observer can latch
    // without a real file path and would wrongly force FileSave(FSF_Ask) on merge/open).
    if (bReloadFile || Path_IsNotEmpty(Paths.CurrentFile)) {
        return false;
    }
    if (!_IsUntitledDocEffectivelyEmpty()) {
        return false;
    }
    // For this buffer FileSave() takes the early exit (no dialog); skip calling it in FileLoad().
    return true;
}
...
bool FileLoad(const HPATHL hfile_pth, const FileLoadFlags fLoadFlags, const DocPos curPos, const DocLn visLn)
{
    ...
    // Normal file open: defer save prompt until needNewTab is known — opening into a new tab must not
    // prompt to save the previous buffer (it stays in its tab). FLF_New / reload still prompt here.
    if (!(fLoadFlags & FLF_DontSave)) {
        bool const deferOpenSave = !(fLoadFlags & FLF_New) && !bReloadFile;
        if (!deferOpenSave) {
            if (_FileLoad_ShouldSkipInitialSavePrompt(bReloadFile)) {
                _MRU_UpdateSession();
                AutoSaveStop();
                ResetFileObservationData(true);
            } else if (!FileSave(FSF_Ask)) {
                return false;
            }
        }
    }
    ...
    if (!bReloadFile) {
        int const iExistingTab = TabDoc_FindTabByPath(hopen_file);
        if (iExistingTab >= 0) {
            Path_Release(hopen_file);
            (void)TabDoc_SwitchToTab(Globals.hwndMain, iExistingTab);
            return true;
        }
        // Same rule as IDM_FILE_NEW / WM_DROPFILES: open in a new tab when the path is not already open
        // and the current buffer is not a disposable empty untitled document.
        ...
        if (!(fLoadFlags & FLF_DontSave)) {
            if (!needNewTab) {
                if (_FileLoad_ShouldSkipInitialSavePrompt(false)) {
                    _MRU_UpdateSession();
                    AutoSaveStop();
                    ResetFileObservationData(true);
                } else if (!FileSave(FSF_Ask)) {
        ...
        if (needNewTab) {
            if (!TabDoc_NewTabForNextDocument(Globals.hwndMain)) {
                Path_Release(hopen_file);
                return false;
            }
        }
...
bool FileSave(FileSaveFlags fSaveFlags)
{
    ...
    bool const bIsEmptyNewFile = Path_IsEmpty(Paths.CurrentFile) && _IsUntitledDocEffectivelyEmpty();
    ...
    if (fSuccess) {
        AutoSaveStop();
        ResetFileObservationData(true);
        TabDoc_RefreshActiveTabFileSnapshot();
    }
...
static bool _LumPad_SendCopyDataOneFile(HWND hwndTarget, LPCWSTR wchPath)
{
    if (!hwndTarget || !IsWindow(hwndTarget) || StrIsEmpty(wchPath)) {
        return false;
    }
    HPATHL hpth = Path_Allocate(wchPath);
    if (!hpth) {
        return false;
    }
    Path_NormalizeEx(hpth, Paths.WorkingDirectory, true, Flags.bSearchPathIfRelative);

    size_t cb = sizeof(np3params);
    cb += (Path_GetLength(hpth) + 1) * sizeof(WCHAR);

    if (s_lpSchemeArg) {
        cb += (StringCchLen(s_lpSchemeArg, 0) + 1) * sizeof(WCHAR);
    }
    size_t const cchTitleExcerpt = StringCchLen(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt));
    if (cchTitleExcerpt) {
    ...
    StringCchCopyW(&params->wchData, Path_GetLength(hpth) + 1, Path_Get(hpth));
...
static bool LumPad_TryStartupForwardToRunningInstance(void)
{
    if (s_flagStartAsTrayIcon || s_flagNewFromClipboard || s_flagPasteBoard) {
        return false;
    }
    bool const haveFiles = Path_IsNotEmpty(s_pthArgFilePath) || (s_cFileList > 0);
    if (!haveFiles) {
        return false;
    }

    HWND hwndPrev = NULL;
    if (!(!EnumWindows(_EnumWndProc, (LPARAM)&hwndPrev) && hwndPrev)) {
        return false;
    }
    ...
    if (!IsWindowVisible(hwndPrev)) {
        SendMessage(hwndPrev, WM_TRAYMESSAGE, 0, WM_LBUTTONDBLCLK);
        SendMessage(hwndPrev, WM_TRAYMESSAGE, 0, WM_LBUTTONUP);
    }
    SetForegroundWindow(hwndPrev);
...
bool ActivatePrevInst(const bool bSetForground)
{
    ...
    // With tabs, "single file instance" still needs cross-process delivery of *new* paths
    // to the running main window; that used to require ReuseWindow=true only.
    if (!(Flags.bReuseWindow || Flags.bSingleFileInstance)) {
        return false;
    }
    ...
    if (!EnumWindows(_EnumWndProc, (LPARAM)&hwnd) && (hwnd != NULL)) {
        ...
        if (hwnd && IsWindowEnabled(hwnd)) {
            ...
            if (Path_IsNotEmpty(s_pthArgFilePath))
            {
                ...
                if (s_cFileList > 1) {
                    for (int fi = 1; fi < s_cFileList; ++fi) {
                        if (s_lpFileList[fi]) {
                            (void)_LumPad_SendCopyDataOneFile(hwnd, s_lpFileList[fi]);
                        }
                    }
                }
...
//=============================================================================
//
//  LumPad_ProcessDeferredMultiOpenTabs()
//
//
static void LumPad_ProcessDeferredMultiOpenTabs(HWND hwnd)
{
    ...
    LumPad_SetStartupBulkRedraw(hwnd, FALSE);
    for (int i = 1; i < s_cFileList; ++i) {
        HPATHL hp = Path_Allocate(s_lpFileList[i]);
        if (!hp) {
            continue;
        }
        Path_NormalizeEx(hp, Paths.WorkingDirectory, true, Flags.bSearchPathIfRelative);
...
{
    if (s_cFileList > 1) {
        /* LumPad: keep file list; load additional files as tabs after main window init */
        return false;
    }
...
LRESULT MsgFileChangeNotify(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if (Path_IsExistingFile(Paths.CurrentFile)) {
        ...
        // wParam!=0: activation of a background tab whose file changed on disk while inactive — show
        // the same prompt even if the global mode is "silent indicator only".
        bool const bInactiveTabDiskDrift = (wParam != 0);
        FILE_WATCHING_MODE const fwmRaw = FileWatching.FileWatchingMode;
        FILE_WATCHING_MODE fwmEffective = fwmRaw;
        if (bInactiveTabDiskDrift && (fwmRaw == FWM_INDICATORSILENT)) {
            fwmEffective = FWM_MSGBOX;
        }
